...
public class HydraStream {
    ...
    private static void hydraKeySchedule(State st, byte[] key, byte[] nonce) {
        /* memcpy(st->key, key, ...) */
        for (int i = 0; i < HYDRA_KEY_SIZE; i++)
            st.key[i] = key[i];
        /* memcpy(st->nonce, nonce, ...) */
        for (int i = 0; i < HYDRA_NONCE_SIZE; i++)
            st.nonce[i] = nonce[i];
    ...
    private static void hydraGenerateBlock(State st, byte[] out) {
        ...
        st.lane[0 * HYDRA_LANE_WORDS + 0] ^= (int) st.counter;
        st.lane[1 * HYDRA_LANE_WORDS + 1] ^= (int)(st.counter >>> 32);
